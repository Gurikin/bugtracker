<cf_template pagename="Authentication page">	
	<cfif structKeyExists(#URL#, 'logout')>
		<cfset application.authServe.signOut()>
	</cfif>
	<cfif structKeyExists(#form#, 'submitSignin')>
		<cfset application.isUserLoggedIn = application.authServe.signIn(form.field_userEmail, form.field_userPassword)>
	</cfif>		
	<cfquery name="signRes">
		SELECT email, fName, count(*) as c FROM user WHERE email = 'vano@gmail.com'
	</cfquery>
	<cfoutput query="signRes">
		#signRes.email# , #signRes.RecordCount#
	</cfoutput>
	<div class="container col-5">
		<cfform class="form-signin" id="formSignin" preservedata="true">
			...
			<cfelse>
				...
				<cfinput type="submit" name="submitSignin" id="submitSignin" value="Login"/>
			</cfif>
		</cfform>
	</div>	
	<cfdump var="#application.authServe.myfoo()#" />
	<cfdump var="#session#" >	
</cf_template>